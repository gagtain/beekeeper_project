    ...
    <div id="orderProductList">
        <div v-for="(orderItem, index) in orderList" :key="orderItem.id" class="product_order_info">
                <div class="w-sto flex jus-sp">
                    <p class="VAG">{{ index+1 }}</p> <p class="VAG">{{ orderItem.price *  orderItem.count}} {{ orderItem.price_currency }}</p>
                </div>
                <div class="flex">
                  ...
                  <div class="info_order_product_div">
                    ...
                    <div class="name_order_product">{{ orderItem.productItem.product.name }} {{ orderItem.productItem.weight ? '[' + orderItem.productItem.weight.weight + 'гр]' : '' }} </div>
                <p>{{ orderItem.price }} {{ orderItem.price_currency }}</p>
